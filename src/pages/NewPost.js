...
export const NewPost = (props) => {
  ...
  const handleChangePic1 = (event) => {
    if (event.target?.files[0]) {
      setFile1({url: URL.createObjectURL(event.target.files[0]), pic: event.target.files[0]})
    }
  };
  ...
  const handleChangePic2 = (event) => {
    if (event.target?.files[0]) {
      setFile2({url: URL.createObjectURL(event.target.files[0]), pic: event.target.files[0]})
    }
  };
  ...
  const handleChangePic3 = (event) => {
    if (event.target?.files[0]) {
      setFile3({url: URL.createObjectURL(event.target.files[0]), pic: event.target.files[0]})
    }
  };
  ...
  const handleChangePic4 = (event) => {
    if (event.target?.files[0]) {
      setFile4({url: URL.createObjectURL(event.target.files[0]), pic: event.target.files[0]})
    }
  };
  ...
  const formSchema = z.object({
    ...
    mainPic: z.any().refine(val => val.length > 0, "Glavna slika je obavezna" )
  })
  ...
  async function submitData(){
    ...
    console.log("Category", selectedCategory, "Location", selectedLocation, "size", selectedSize, "color", selectedColor)
       const { error: cardError, data: cardData } = await supabase
         .from('Card')
         .insert({ title: newPost.title, description: newPost.description, picture: "/mainPic" }).select('id').single()
    ...
    <div className="newPostMain">
      ...
      <form onSubmit={handleSubmit(submitData)}>
        <div className='newPostElements'>
          <div className='elementsLeft'>
            <div className='elementOne'>
              <label >Naslov</label>
              <input type='text' className='newPostNaslov' {...register("title")} name='title' onChangeCapture={handleChangeForm}/>
              { errors.title && (
              ...
              <Select options={optionsCategory} isMulti placeholder="Odaberite kategoriju"
                ...
                className='newPostKategorija' name='categoryId' onChange={handleChangeCategory}
              />
              <label>Boja</label>
              <Select options={optionsColor} isMulti placeholder="Odaberite boju"
                ...
                className='newPostKategorija' name='colorId' onChange={handleChangeColor}
              />
            </div>
            <div className='elementTwo'>
              <label>Lokacija</label>
              <Select  options={optionsLocation} isMulti placeholder="Odaberite lokaciju" 
                styles={{
                  control: (baseStyles) => ({
                    ...baseStyles,
                    borderRadius: "0",
                ...
                className='newPostKategorija' name='locationId' onChange={handleChangeLocation}
              />
              ...
              <Select options={optionsSize} isMulti placeholder="Odaberite veličinu"
                ...
                className='newPostKategorija' name='categoryId' onChange={handleChangeSize}
              />
            </div>
          </div>
          <div className='elementsRight'>
            <label  >Opis</label>
            <textarea type='text' className='newPostOpis' maxLength={1000} {...register("description")} name='description' onChangeCapture={handleChangeForm}/>
            { errors.description && (
        ...
        <div className='newPostImages'>
          <div className='newPostLeft'>
            <div className='mainImage imgOne'>
              {file1.url ? (
                <img className='mainImageInside' src={file1.url} onClick={handleClick} />
              )
                : (
                  <button className='mainImageInside' type='button' onClick={handleClick}>Dodaj<br />glavnu<br />fotografiju</button>
                )}

              <input type="file"
                
                id="hiddenFileInput1"
                style={{ display: 'none' }}{...register("mainPic")}
                onChangeCapture={handleChangePic1}
                
              />
            </div>
            { errors.mainPic && (
                <p className='errorMsg'>{errors.mainPic.message}</p>
              )}
              
            <div className='mainImage imgOne'><div className='imageInside'>

              {file2.url ? (
                <img className='imageInside' src={file2.url} onClick={handleClick2} />
              )
                : (
                  <button className='buttonInside' type='button' onClick={handleClick2}>
                    <span>
          ...
          <div className='newPostRight'>
            <div className='mainImage imgTwo'><div className='imageInside'>
              {file3.url ? (
                <img className='imageInside' src={file3.url} onClick={handleClick3} />
              )
                : (
                  <button className='buttonInside' type='button' onClick={handleClick3}>
                    <span>
                      <svg xmlns="http://www.w3.org/2000/svg"
                        className='plusIcon'
                        viewBox="0 0 256 256">
            ...
            <div className='mainImage imgTwo'><div className='imageInside'>
              {file4.url ? (
                <img className='imageInside' src={file4.url} onClick={handleClick4} />
              )
                : (
                  <button className='buttonInside' type='button' onClick={handleClick4}>
                    <span>
                      <svg xmlns="http://www.w3.org/2000/svg"
                        className='plusIcon'
                        viewBox="0 0 256 256">
        ...
        <div className='newPostButton'>
          <button className='addButton' type='submit' disabled={isSubmitting}>Dodaj objavu</button>
        </div>
      </form>
    </div>)
    :
    (<div>Za unos oglasa trebate biti prijavljeni!</div>)}
    </>
  );
};